...
function Home(props) {
  const list0 = props.school.slice(0, 3).map((c) => {
    return (
      <div className="mx-2">
  ...
  const list1 = props.myclass.slice(0, 3).map((c) => {
    return (
      <div className="mx-2">
  ...
  const albumList = tentativeAlbum.slice(0, 4).map((album) => {
    return (
      <div className="col ps-4 pb-4">
        <img
          className="w-100 pb-1 border"
          src={album.img + ".jpg"}
    ...
    <div>
      ...
      <div className="pb-5">
        ...
        <FullCalendar
          ...
          locale="ja" // 日本語化
          events={[
            { title: "遠足", start: "2022-05-06" },
            { title: "やがえふ", start: "2022-06-19" },
            { title: "運動会", start: "2022-09-17" },
            { title: "音楽祭", start: "2022-10-24" },
            { title: "授業参観", start: "2023-12-20" },
            { title: "閉庁", start: "2022-12-29", end: "2023-01-04" },
            { title: "社会見学", start: "2023-01-12" },
            { title: "修学旅行", start: "2023-02-17", end: "2023-02-19" },
          ]}
        />
      </div>
      <p id="page-top">
      ...
      <footer>
        <div className="footer mt-3 border-top">
          <ul className="menu">
            <li className="pb-1 footer-link" style={{ fontSize: "14px" }}>
              <Link to="/Preparation">〇〇小学校ホームページ</Link>
            </li>
            <li className="pb-1 footer-link" style={{ fontSize: "14px" }}>
              <Link to="/Preparation">サイトの制作にあたって</Link>
            </li>
            <li className="pb-1 footer-link" style={{ fontSize: "14px" }}>
              <Link to="/DesignProposal">プロジェクト案</Link>
            </li>
          </ul>
          <p className="pt-4 pb-3" style={{ fontSize: "13px" }}>
            &copy; 2023 Oya class
          </p>
        </div>
      </footer>
